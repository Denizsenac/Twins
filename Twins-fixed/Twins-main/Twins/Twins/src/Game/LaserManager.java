...
public class LaserManager {
    ...
    // FIX: place a packed laser at an exact position (used when loading a saved game)
    public void spawnPackedLaserAt(int x, int y) {
        if (packCount < packs.length) {
            packs[packCount] = new PackedLaser(x, y);
            packCount++;
        }
    }

    // FIX: accessors for saving packed laser state
    public int[] getPackXArray() {
        int[] arr = new int[packCount];
        for (int i = 0; i < packCount; i++) arr[i] = packs[i].getX();
        return arr;
    }

    public int[] getPackYArray() {
        int[] arr = new int[packCount];
        for (int i = 0; i < packCount; i++) arr[i] = packs[i].getY();
        return arr;
    }

    public boolean[] getPackCollectedArray() {
        boolean[] arr = new boolean[packCount];
        for (int i = 0; i < packCount; i++) arr[i] = packs[i].isCollected();
        return arr;
    }
}
